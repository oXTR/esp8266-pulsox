...
class MAX30102():
    ...
    # and, for i2c, your scl is Pin 5, your sda is Pin 4, and freq is 400k
    def __init__(self, 
                 clock_pin=5, 
                 data_pin=4, 
                 frequency=400000,
...
#        self.bus = smbus.SMBus(self.channel)
        self.bus = smbus.SMBus(scl=Pin(clock_pin), sda=Pin(data_pin), freq=frequency)
    ...
    def read_fifo(self):
        ...
        e = self.bus.read_i2c_block_data(self.address, REG_FIFO_DATA, 6)

        # mask MSB [23:18]
        red_led = (e[0] << 16 | e[1] << 8 | e[2]) & 0x03FFFF
        ir_led = (e[3] << 16 | e[4] << 8 | e[5]) & 0x03FFFF
    ...
    def read_fifo_bytes(self, num_samples=32):
      ...
      # read 1 byte from registers (values are discarded)
        ...
        e = self.bus.read_i2c_block_data(self.address, REG_FIFO_DATA, num_samples*6)
